		<div class="col m4 s6 l4 --general-t --siderbar">
				...
				<div class="collection sml brd4 boxsh nomargin">
					@if(Auth::check())
						...
						<a href="#" class="collection-item blue-text tooltipped" data-position="left" data-tooltip="Вы уже благодарили пользователя"> <!-- Лайкнул -->
							Спасибо <span class="badge blue-text">{{ App\Tasks::getUserLikeCount(Request::segment(2)) }}</span><i class="material-icons">favorite_border</i>
						</a>
						@else
						<a href="/profile/add_like/{{ Request::segment(2) }}" class="collection-item blue-text tooltipped" data-position="left" data-tooltip="Сказать спасибо">
							Спасибо <span class="badge blue-text">{{ App\Tasks::getUserLikeCount(Request::segment(2)) }}</span><i class="material-icons">favorite_border</i>
						</a>
						@endif
					@else
			...
			<div class="--group --share-btns">
				<div class="collection with-header sml brd4 boxsh nomargin">
					...
					@endif
					<a href="/portfolio/{{ Request::segment(2) }}" class="collection-item blue-text">
						Примеры работ <i class="material-icons">perm_media</i>
					</a>
					<!--links-->
					<div class="divider"></div>
					@if(App\Tasks::getUserParamWithId(Request::segment(2),'user_fb')!='')
					<a href="//{!! App\Tasks::getUserParamWithId(Request::segment(2),'user_fb') !!}" class="collection-item blue-text" target="_blank">
						Facebook <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Tasks::getUserParamWithId(Request::segment(2),'user_vk')!='')  
					<a href="//{!! App\Tasks::getUserParamWithId(Request::segment(2),'user_vk') !!}" class="collection-item blue-text" target="_blank">
						Вконтакте <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Tasks::getUserParamWithId(Request::segment(2),'user_instagram')!='')  
					<a href="//{!! App\Tasks::getUserParamWithId(Request::segment(2),'user_instagram') !!}" class="collection-item blue-text" target="_blank">
						Instagram <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Tasks::getUserParamWithId(Request::segment(2),'user_web')!='')  
					<a href="//{!! App\Tasks::getUserParamWithId(Request::segment(2),'user_web') !!}" class="collection-item blue-text" target="_blank">
