										...
										<div class="card-action">
											...
											<div class="right">
												<span class="--time grey-text text-darken-1 veralmidd">
													<strong>@if($t->status==0) {!! '<strong class="green-text">Актуально</strong>' !!} @else {!! '<strong class="orange-text">Закрыто</strong>' !!} @endif</strong>: {{ $t->start_date }}
												</span>
												@if($t->bujet!='')
												<span class="--sum yellow veralmidd">{{ $t->bujet }} {{ App\Lang::getTrans('valuta', Config::get('app.locale'))}}</span>
												@endif
											</div>
										</div>
									</div>
