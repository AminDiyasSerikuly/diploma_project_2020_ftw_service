...
{{--							<span class="veralmidd __bold orange-text">{{ App\Tasks::getAccountAmount(Auth::user()->id) }} @if(App\Profile::getUserParam('user_current')!=''){{ App\Profile::getUserParam('user_current') }}@elseВалюта не задана @endif</span>--}}
					...
					<a href="/my" class="collection-item black-text @if(Request::segment(1)=='my' && Request::segment(2)=='') {{ 'active' }} @endif">
						Моя страница <i class="material-icons">person</i>
					</a>
					<a href="/my/tasks" class="collection-item black-text @if(Request::segment(2)=='tasks') {{ 'active' }} @endif">
						Мои задачи <i class="material-icons">fiber_manual_record</i>
					</a>
					<a href="/my/edit" class="collection-item black-text @if(Request::segment(2)=='edit') {{ 'active' }} @endif">
						Редактировать <i class="material-icons">edit</i>
					</a>
					<a href="/my/settings" class="collection-item black-text @if(Request::segment(2)=='settings') {{ 'active' }} @endif">
			...
			<div class="--group --share-btns">
				<div class="collection with-header sml brd4 boxsh nomargin">
					...
					@endif
					<a href="/portfolio/{{ Auth::user()->id }}" class="collection-item black-text">
						Примеры работ <i class="material-icons">perm_media</i>
					</a>
					<!--links-->
					<div class="divider"></div>
					@if(App\Profile::getUserParam('user_fb')!='')
					<a href="{{ App\Profile::getUserParam('user_fb') }}" class="collection-item black-text" target="_blank">
						Facebook <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Profile::getUserParam('user_vk')!='')
					<a href="{{ App\Profile::getUserParam('user_vk') }}" class="collection-item black-text" target="_blank">
						VK <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Profile::getUserParam('user_instagram')!='')
					<a href="{{ App\Profile::getUserParam('user_instagram') }}" class="collection-item black-text" target="_blank">
						Instagram <i class="material-icons">link</i>
					</a>
					@endif
					@if(App\Profile::getUserParam('user_web')!='')
					<a href="{{ App\Profile::getUserParam('user_web') }}" class="collection-item black-text" target="_blank">
					...
					<a href="/my/settings#links" class="collection-item black-text">
